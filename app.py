            ...
            if btn_entrar:
                try:
                    ...
                    if "Name or service not known" in err_msg or "-2" in err_msg:
                        st.error("⚠️ Falha de conexão: Não foi possível alcançar o Supabase. Verifique a URL e a Key cadastradas nos Secrets do Streamlit Cloud.")
                    else:
                    ...
                    with st.container(border=True):
                        ...
                        with c_m2:
                            if item["status"] == "realizada":
                                status_tag = '✅ REALIZADA'
                            elif item["status"] == "falta_cobrada":
                                status_tag = '❌ FALTA COBRADA'
                            else:
                                status_tag = '🔵 AGENDADO'
