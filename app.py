        ...
        with tab_caixa:
            ...
            with c_add:
                ...
                with st.form("form_trans"):
                    ...
                    cat_t = st.selectbox("Categoria", ["Mensalidade", "Combustível", "Equipamento", "Curso/Educação", "Aluguel Espaço", "Outros"])
                    desc_t = st.text_input("Descrição")
                    dt_trans = st.date_input("Data da Transação", value=hoje)

                    if st.form_submit_button("Registrar Transação", type="primary", use_container_width=True):
                        preparar_cliente()
            ...
            with c_resumo:
                st.markdown("### Extrato e Saldo")
                preparar_cliente()
                res_t = supabase.table("transacoes").select("*").eq("user_id", user_id).order("data_transacao", desc=True).execute()
                transacoes = res_t.data if res_t.data else []

                tot_receitas = sum([float(t.get("valor") or 0.0) for t in transacoes if t.get("tipo") == "Receita"])
                tot_despesas = sum([float(t.get("valor") or 0.0) for t in transacoes if t.get("tipo") == "Despesa"])
                saldo_liquido = tot_receitas - tot_despesas

                m1, m2, m3 = st.columns(3)
                m1.metric("Total Receitas", f"R$ {tot_receitas:.2f}")
                m2.metric("Total Despesas", f"R$ {tot_despesas:.2f}")
                m3.metric("Saldo Líquido", f"R$ {saldo_liquido:.2f}")
