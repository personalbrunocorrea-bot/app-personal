...
# --- APLICAÇÃO PRINCIPAL ---
    ...
    # ==========================================
    # 📌 MELHORIA 1: ALERTAS INTELIGENTES NO TOPO
    # ==========================================
    ...
    for al in alunos_todos:
        # Alerta 1: Pacote acabando (<= 2 aulas restantes)
        ...
        # Alerta 2: Saldo pendente após o vencimento
    ...
    # 2. AGENDA SEMANAL COM SUPORTE A RECORRÊNCIA E MODO COMPACTO
        ...
        # ==========================================
        # 📌 MELHORIA 6: MODO COMPACTO/MOBILE
        # ==========================================
        modo_exibicao = st.radio("Modo de Visualização", ["🖥️ Grade Completa (7 Dias)", "📱 Modo Lista/Celular"], horizontal=True)

        alunos = alunos_todos
        mapa_alunos_id = {a["id"]: a for a in alunos}
        
        # ABA DE AGENDAMENTOS (AVULSO E RECORRENTE)
        ...
        # ==========================================
        # 📌 MELHORIA 3: AGENDAMENTO RECORRENTE
        # ==========================================
        with tab_recorrente:
            if not alunos:
                st.warning("Cadastre um aluno primeiro.")
                ...
                with st.form("form_agendar_recorrente"):
                    ...
                    if st.form_submit_button("🔁 Gerar Agenda Recorrente"):
                        preparar_cliente()
        ...
        # EXIBIÇÃO: MODO GRADE
        if modo_exibicao == "🖥️ Grade Completa (7 Dias)":
                ...
                with col:
                    ...
                    if not agendamentos_dia:
                        ...
                        for item in agendamentos_dia:
                            ...
                            elif item["status"] == "falta_isenta": status_tag = "🟡"
        ...
        # EXIBIÇÃO: MODO LISTA / CELULAR (CARDS)
        else:
                ...
                for item in agendamentos_dia:
                    ...
                    with st.container(border=True):
                        ...
                        with c_m2:
                            ...
                            if aluno_data:
                                st.caption(f"Aulas rest: {aluno_data.get('aulas_restantes', 0)}")
                        with c_m3:
                            ca1, ca2, ca3 = st.columns(3)
                            if ca1.button("✅ Presença", key=f"mp_{item['id']}"):
                                preparar_cliente()
                                supabase.table("agendamentos").update({"status": "realizada"}).eq("id", item["id"]).execute()
                                if aluno_data:
                                    upd = {"presencas": (aluno_data.get("presencas") or 0) + 1}
                            ...
                            if ca2.button("❌ Falta", key=f"mfc_{item['id']}"):
                                preparar_cliente()
                                supabase.table("agendamentos").update({"status": "falta_cobrada"}).eq("id", item["id"]).execute()
                                if aluno_data:
                                    upd = {"faltas": (aluno_data.get("faltas") or 0) + 1}
                            ...
                            if ca3.button("🗑️", key=f"mdel_{item['id']}"):
                                preparar_cliente()
                                supabase.table("agendamentos").delete().eq("id", item["id"]).execute()
                                st.rerun()

    # 3. PERFIL DO ALUNO COM FILTRO E BUSCA RÁPIDA
        ...
        if not alunos:
            ...
            # ==========================================
            # 📌 MELHORIA 6: BUSCA E FILTRO RÁPIDO
            # ==========================================
